...
public class Terminal {
  ...
  public static TerminalExample toExample(String ip, String terminalSegment) {
    ...
    if (terminalSegment != null) {
      criteria.andTerminalSegmentEqualTo(terminalSegment);
    }
    example.setOrderByClause("ip_3 asc, ip_2 asc, ip_1 asc, ip_0 asc");
    return example;
  }

  public static TerminalExample switchExample() {
    TerminalExample example = new TerminalExample();
    example.createCriteria().addCriterion("terminal_type in (select network_parameter_id from network_parameter where network_parameter_group = 0 and network_parameter_use < 4 and network_parameter_value in ('switch', 'firewall'))");
